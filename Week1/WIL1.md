...
백틱으로 감싼 문자열로, 이스케이프 문자 없이 개행문자만으로도 줄바꿈 할 수 있다. 템플릿 리터럴 앞에 `String.raw`를 붙이면 이스케이스 문자를 그대로 출력할 수 있다. 문자열 리터럴로는 `\\n`이 된다. 
...
인자로 넘겨진 타입에 따라 함수는 다르게 작동한다. 원시 값은 값에 의한 호출, 객체 값은 참조에 의한 호출이다.
...
함수 리터럴이라는 것이 있다. 함수 리터럴은 다른 `function add(a, b) { ... }` 같은 함수와 다르게 `function(a, b)` 처럼 이름이 존재하지 않는다. 그래서 익명 함수라고도 한다. 함수 리터럴을 변수에 할당하면 함수처럼 쓸 수 있다. 가령 `var add = function(a, b) { ... }` 같은 식으로 쓰면 기존에 함수 선언문에서 했던 것처럼 가능하다. 사용법도 똑같다. 내부적으로는 둘 다 동일하게 작동하기 때문이다. 단 차이점은 함수 리터럴은 선언문이 아니므로 호이스팅이 적용되지 않는다는 것이다.

### 스코프

스코프는 유효 범위라고 한다. 프로그래밍 언어의 스코프에는 두 가지 방식이 있다. 하나는 프로그램 신택스에 따르는 렉시컬 스코프이고, 다른 하나는 런타임 중에 결정되는 다이나믹 스코프이다. 대부분의 언어는 컴파일 타임에 스코프가 결정되는 렉시컬 스코프를 사용한다. JS도 마찬가지다. 이 스코프에 따라 변수를 두 가지로 나눌 수 있다. 전역 변수와 지역 변수다.

전역 변수와 지역 변수가 충돌하면 전역 변수가 지역 변수에 의해 가려진다.

 스코프와 관련있는 개념으로 `let`과 `const`를 들 수 있다. 둘다 블록 스코프를 갖는 선언자로, 중괄호 안에서만 유효하다. `let`은 변수, `const`는 상수 역할을 한다. 예전에 `var`만 선언자로 사용되었을 때는 함수 스코프를 갖기 때문에 함수가 아니라면 전역변수 취급을 받았다. 하지만 `let`은 블록 스코프를 갖기 떄문에 함수 블록이 아닌 if, for, while 등등 모든 중괄호를 사용하는 블록을 스코프로 갖는다. `const`의 경우 상수 역할을 하기 때문에 선언과 동시에 값을 할당해야 한다. 그리고 이후에 값을 변경할 수 없다. 단 이 값이 객체이거나 배열이라면 객체의 프로퍼티나 배열의 요소는 변경 가능하다.

요약하자면 이렇다.

1. 중복 선언 가능 여부 : `var` 가능, `let` 불가능, `const` 불가능
2. 재할당 가능 여부 : `var` 가능, `let` 가능, `const` 불가능
3. 스코프 : `var` 함수 스코프, `let` 블록 스코프, `const` 블록 스코프
4. 호이스팅 : `var` 실행 전 선언 및 초기화 / `let`, `cost` 실행 전 선언, 초기화는 실행 후 선언문에서, 그 전에는 TDZ에 존재

https://dev-coco.tistory.com/112

### 객체의 메서드

JS의 객체는 특이하게 맵같은 구조로 되어있어서 메서드는 프로퍼티에 함수를 할당하는 식으로 만들어준다. 그리고 일반적인 클래스-인스턴스 개념이 아니라서 빵판과 빵처럼 생각하지 않아도 된다. 그래서 객체에 메서드를 추가로 덧붙이는 것도 가능하다. 아까 함수 리터럴해서 했던 것처럼 `object.add = function(a, b) { ... }` 식으로 추가할 수 있다.

생긴게 이래서 이게 객체지향이 맞나 싶을 수도 있겠지만 JS도 엄연히 객체지향 언어다. 일반적인 클래스 기반 객체지향 언어가 아니라 프로토타입 기반 객체지향일 뿐이다. 

위에서 빵판과 빵처럼 생각하지 않아도 된다고 했지만 그렇게 생각해도 된다. 생성자 추가하고 `new`로 똑같이 인스턴스를 만들 수 있다. 좀 특이한 건 메서드도 프로퍼티라서 생성자의 인자로 다르게 넘겨질 수 있다.

### 배열

특이한 건 배열도 객체라는 거다. 객체를 잘 만들어서 우리가 아는 배열처럼 동작하게 만든 것이라 할 수 있다. 그래서 인덱스에 문자열 넣어도 돌아간다. 희소 배열이라는 것도 있는데 인덱스가 1, 2, 3, 4가 아니라 3이 없을 수도 있다.


### 나머지

그 외 문법적인 / 디테일한 부분들은 생략한다.
